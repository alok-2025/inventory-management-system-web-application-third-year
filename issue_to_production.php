<?php 
include('tbl_conn_files/issue_to_production_conn.php'); 
include('session.php'); 
?>
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="styles/issue_to_production_style.css">
...
<body>
    ...
    $(document).ready(function () {
        $("#btnExportXlsx").click(function () {
            ...
            TableToExcel.convert(table[0], {
                name: `CTG_Issue_To_Production_Table.xlsx`,
                sheet: {
                    name: 'CTG_Issue_To_Production_Table'
                }
            });
        });
    });
	...
	<div class="ctg_links">
	<a href="add_issue_to_production.php">Add Issue</a>
	<button type="button" class="btn" id="btnExportXlsx">Export Table</button><br><br>
	<?php } ?>
	</div>
	<div class="table_wrapper">
	<table id="ctg_issue_to_production" border="1">
	<thead>
		<tr>
		<th>Date Created</th>
		<th>Voucher Number</th>
		<th>Department</th>
		<th>Production BatchNo</th>
		<th>Created By</th>
		<th>Status</th>
		<?php if ($_SESSION['role'] == 'Production Manager' or $_SESSION['role'] == 'Warehouse Manager') { ?>
		<th>Action</th>
		<?php } ?>
	</tr>
	</thead>
	<tbody>
		...
		<?php
		$query = "SELECT * FROM issue_to_production";
		...
		?>
			<tr>
				...
				<td><?php echo $row['production_batchno']; ?></td>
				<td><?php echo $row['created_by']; ?></td>
				<td><?php echo $row['itp_status']; ?></td>
				<?php if ($_SESSION['role'] == 'Production Manager' or $_SESSION['role'] == 'Warehouse Manager') { ?>
					<td><a href="update_issue_to_production.php?updateid=<?php echo $row['id']; ?>">View</a></td>
				<?php } ?>
			</tr>
		<?php	}
			}
	...
	<tfoot>
		<tr>
			<td colspan="7">
				<?php 
				// * summation field
				$result = mysqli_query($conn, "SELECT count(*) FROM issue_to_production");
				while ($rows = mysqli_fetch_array($result)){?>
				<?php echo "Total Issues: " . $rows['count(*)']; ?> 
				<?php
				}
				?>
			</td>
