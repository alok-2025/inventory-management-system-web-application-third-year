...
<?php
// Include delete_soap_costing_conn file
require_once "crud_conn_files/delete_soap_costing_conn.php";

// Initialise the material variable
$material = '';
...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    // Fetch the material using the function from the connection file
    $material = getMaterialById($conn, $param_id);
    
    // Prepare a delete statement
    $query = "DELETE FROM soap_costing WHERE id = ?";
    ...
    if (empty(trim($_GET["id"]))) {
        ...
        // If coming from GET request, fetch the material to display in the confirmation
...
<html lang="en">
<head>
    <title>CTG Limited: Remove Soap Material</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <link rel="stylesheet" type="text/css "href="styles/style.css">
    <link rel="stylesheet" type="text/css" href="styles/delete_soap_costing_style.css">
</head>
<body>
    ...
    <div class="del_costing">CTG Soap Costing</div>
    <div class="header">
        <h2>Material deletion confirmation</h2>
    </div>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="del_warning"><img src="icons/warning.png" alt="Warning">
                <input type="hidden" name="id" value="<?php echo trim($_GET["id"]); ?>"/>
                <p>Are you sure you want to remove <?php echo htmlspecialchars($material); ?>?</p><br>
                    <button type="submit" value="yes" class="btn">Yes</button>
                    <a href="soap_costing.php" value="no"class="btn">No</a>
                    </div>
            </div>
        </form>
    <div class="footer">
