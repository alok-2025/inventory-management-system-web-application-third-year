<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        $qty = $_POST['qty'];
        $query = mysqli_query($conn, "UPDATE ctg_products SET prod_name='$prod_name', prod_category='$prod_category', prod_price='$prod_price',qty='$qty' WHERE id='$id'");
        if($query){
...
<head>
    ...
    <title>CTG Limited: Edit Product Info</title>
</head>
<body>
    ...
    <form method="post">
        ...
        include ('crud_conn_files/conn.php');
            $id = $_GET['updateid'];
            $query = mysqli_query($conn, "SELECT * FROM ctg_products WHERE id='$id'");
            while ($row = mysqli_fetch_array($query)) {
        ?>
                        
        <div class="input-group">
            <label>Product</label>
            <input type="text" value="<?php echo $row["prod_name"]?>" name="prod_name">
        </div>
        <div class="input-group">
            <label>Category</label>
            <input type="text" value="<?php echo $row["prod_category"]?>" name="prod_category">
        </div>
        <div class="input-group">
            <label>Price</label>
            <input type="text" value="<?php echo $row["prod_price"]?>" name="prod_price">
        </div>
        <div class="input-group">
